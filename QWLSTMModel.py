...
class QWLSTMModel:
    ...
    def __init__(self, hs=64, quantile: float = 0.05, device='cpu'):
        """
    ...
    def fit(self, x, y, weight, tau=0.5, d=False, batch_size=100, n_iter=500, lr=1e-3, tol=1e-5, verbose=True):
        ...
        self.fnet = LSTMModel(p, 1, self.hs).to(self.device)
        ...
        for i_iter in range(n_iter):
            ...
            if tau is None:       
                ...
                loss2 = torch.mean(self.quantile_loss(tmp_y, tmp_fx.ravel(), self.q) * tmp_w.T) * n / (n - 1)
